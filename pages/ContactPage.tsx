...
const ContactPage: React.FC = () => {
  ...
  // You can find these in your EmailJS account dashboard: https://dashboard.emailjs.com/
  const SERVICE_ID = 'service_cih9cqz';
  const TEMPLATE_ID = 'template_350d2ai';
  const PUBLIC_KEY = 'gq55ZDfdy9UJ04AWJ';
    ...
    (window as any).emailjs.sendForm(SERVICE_ID, TEMPLATE_ID, form.current, PUBLIC_KEY)
      .then(
        ...
        (error: any) => {
          ...
          const errorText = error.text || 'An unknown error occurred. Please check your EmailJS configuration and try again.';
            ...
            <div className="bg-white dark:bg-slate-800 p-10 rounded-lg shadow-lg">
              <form ref={form} onSubmit={handleSubmit} className="grid grid-cols-1 gap-y-6">
                ...
                <div>
                  ...
                  <textarea id="message" name="message" rows={4} required className="block w-full shadow-sm py-3 px-4 rounded-md bg-slate-100 text-slate-900 placeholder-slate-500 border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white" placeholder="Message"></textarea>
                </div>
                <div>
